...
EXPERIMENT_DATA_ARGS = {
    "faces": {
        ...
        "image_path": "./notebooks/images/messi4.jpg",
...
delta = 9
fig, axs = plt.subplots(6,cant,figsize=(10,10))
for e in range(1,7):
    print(e)
    ...
    for i in range(cant):  
        W = inicial + ((final-inicial)*(2*i/delta))#i
        lat = [W.reshape([1,18,512]).float()]  
        #np.savez(f'./results/latente2{e}/{i}.npz', w=lat[0].cpu())   
        ...
        res = (lat_image[0])[0] 
        axs[e-1,i].set_axis_off()  
        axs[e-1,i].imshow(res) 
        
        outputs_path = fr'./results/temp/imagen{e}/{i}.jpg'
        res.save(outputs_path) 
    print(f'## {e} ##################################################')
fig.tight_layout()
plt.savefig("./results/temp/all.png", transparent=True)
