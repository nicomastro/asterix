...
def mkdirs(subject, e, direction):
    ...
    path = fr'./results/new/{subject}/imagen{e}_{direction}'
...
def store_intermediate_results(results_batch, results_latent, results_deltas,
    ...
    for idx in range(y_hat.shape[0]):
        ...
        results_deltas[idx].append([w[idx].cpu().numpy() if w is not None else 
                                        None for w in weights_deltas])
...
def interpolate_samples(latent, latent_editor,img_path,weights_deltas,n,augment):
    ...
    for k in range(augment):
        for e in range(1,7):
            # m7 direcciones mixtas para augmentation, m2 las puras
            sav = sorted(glob.glob(f'./models/m7/m7_{k}/REG{e}/*.sav'), 
                         key=get_key)
            #sav = sorted(glob.glob(f'./models/m2/REG{e}/*.sav'), key=get_key)
            sav_vect = [x for x in sav]
            vect = latent.reshape([18,512])
            inicial = vect.clone().reshape([18,512])
            ...
            final = torch.tensor(np.asarray(f*18), 
                                device=torch.device('cuda')).reshape([18,512])
            ...
            for i in range(n):  
                if e in [2,3]:
                    W = inicial + ((final-inicial)*(1.5*i/delta)) 
                elif e in [4,6]:
                    W = inicial + ((final-inicial)*(args.hneg*i/delta))
                else:
                ...
                res = transforms.Compose([transforms.Resize((256, 256)),
...
def fit_models(imgs, latents, n, augment, k,models):
    ...
    # Labels 2DLDA
    y_2d = np.tile(np.arange(1,7).repeat((n-1)), augment)
...
parser.add_argument('--hneg', type=float, default=1)
...
subjects = 123 if args.dataset == 'CK' else 1
for s in range(subjects):
    ...
    for i, seq in enumerate(image_path):
        ...
        imgs, latents = interpolate_samples(lat,lat_editor,neutral,weights_deltas,
                                            args.frames,args.augment)
        if args.models in ["PCA","LDA"]:
            fit = fit_models_pca
        else:
            fit = fit_models

        Z_imgs, Z_lats, y, models, gmean, std  = fit(imgs, latents, args.frames,
                                                     args.augment,args.k,args.models)
...
if subjects > 1:
    with open("results.txt", 'a') as f:
        res = (fr'Acc. Imgs = {acc/nframes} | Acc. lats = {acc2/nframes} | Total Sequences = {nframes} | k = {args.k} | n = {args.augment} | hpos = {args.hpos} | hneg = {args.hneg} | frames = {args.frames} | models = {args.models}')
        f.write(res)
        f.write('\n')
        print(res)
