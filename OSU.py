...
def encoder(in_path, out_path):
    # we get our images
    #nutral
    p = Path(in_path)
    pictures = os.listdir(p)
    print('copy image normal')
    src_files = os.listdir(p)
    for file_name in src_files:
        full_file_name = os.path.join(p, file_name)
        if os.path.isfile(full_file_name):
              shutil.copy(full_file_name, out_path+'/normal')
    ...
    for k in range(len(pictures)):
        ...
        res = (lat_image[0])[0]
        print('saving encoder hyper')
        res.save(out_path+'/hyper/'+'%s.png' % (pictures[k].replace('.png', '')))
        print('looooooooooooooooooooooooooooooooong = ',len(weights_deltas1))
        lat_image2 = latent_editor._latents_to_image(all_latents= latent1, weights_deltas =[None]* len(weights_deltas1))           
        res = (lat_image2[0])[0]
        print('saving encoder encoder')
        res.save(out_path+'/e4e/'+'%s.png' % (pictures[k].replace('.png', '')))
        print('----------------------------------------------------------------------------') 
...
print('------------------------------------------------------------')        
def encoder_stylemix(in_path, out_path):
    # we get our images
    #nutral
    p = Path(in_path)
    pictures = os.listdir(p)
